...
def test_staged_order3_geqoe_parity_vs_python() -> None:
    eq0 = _to_eq_state(_reference_cart_state())
    dt = np.array([0.0, 30.0, 120.0, 600.0], dtype=float)

    coeffs_cpp = prepare_taylor_coefficients_cpp(eq0, _J2, _Re, _mu, order=3)
    y_cpp, stm_cpp, map_cpp = evaluate_taylor_cpp(coeffs_cpp, dt)

    coeffs_py = py_core.prepare_taylor_coefficients(y0=eq0, p=(_J2, _Re, _mu), order=3)
...
def test_staged_order3_cartesian_parity_vs_python() -> None:
    y0 = _reference_cart_state()
    tspan = np.array([0.0, 30.0, 120.0, 600.0], dtype=float)

    coeffs_cpp, peq_py_0_cpp = prepare_cart_coefficients_cpp(y0, _J2, _Re, _mu, order=3)
    y_cpp, stm_cpp = evaluate_cart_taylor_cpp(coeffs_cpp, peq_py_0_cpp, tspan)

    coeffs_py, peq_py_0_py = py_core.prepare_cart_coefficients(y0_cart=y0, p=(_J2, _Re, _mu), order=3)
