...
\textbf{Response:} We agree. Three specific improvements:
\begin{enumerate}[nosep]
...
Section~5.2: ``In the integrands that follow, all slow variables---including
$\omega$, $g$, $Q$---are evaluated at their mean values, consistent with the
first-order averaging prescription.''

\textbf{Changes:} Section~5.2 opening: new paragraph.
...
\textbf{Response:} A derivation sketch has been added after the corrected
$w_h$ formula showing the chain from the exact GEqOE definitions through
$F_h$, $\hat z$, and $h$ to the first-order result.

\textbf{Changes:} Section~5.2: new derivation paragraph after
Eq.~(wh\_j2\_first).
...
Appendix~A discussion, noting that Brouwer is a purely analytical theory
with no mean-only output mode, and that the DSST mean-drift decomposition
shows short-period reconstruction dominates the total error budget.
