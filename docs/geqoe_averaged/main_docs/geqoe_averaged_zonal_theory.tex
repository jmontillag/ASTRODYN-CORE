...
The GEqOE state vector admits exact first-order orbit averaging in the
generalized eccentric longitude $K$, yielding a finite Fourier-harmonic
mean-element system in the single slow angle $\omega = \Psi - \Omega$.
A direct residue decomposition exploiting the fixed pole structure of the
$dM/dF$ Jacobian ($F = e^{if}$) produces the short-period kernels as
closed-form functions of $(q,Q,F)$---verified through $J_5$ and
conjectured for all zonal degrees---exact in eccentricity and precomputed
once by symbolic algebra.
An equinoctial variant of the short-period map constructs corrections
directly in $(p_1,p_2,q_1,q_2)$ via complex reduced rates, eliminating
the polar-coordinate singularity at $g \to 0$ and $Q \to 0$.
At runtime the short-period map evaluates algebraic expressions with no
quadrature or iteration (except the Kepler equation), at a per-epoch cost
comparable to Brouwer--Lyddane.
Validation against osculating Taylor integration achieves sub-kilometer
position reconstruction across all eccentricities and inclinations
over multi-day arcs.
A controlled comparison against DSST and Brouwer--Lyddane
(identical gravity model) shows that the GEqOE first-order theory
consistently outperforms both references in osculating position accuracy
across all test regimes, despite lacking second-order secular corrections.
\end{abstract}
...
for space surveillance.  The present work introduces a new analytical theory
that combines three ingredients not previously assembled: Generalized
Equinoctial Orbital Elements (GEqOE) that absorb zonal perturbative effects
into the element definitions, orbit averaging in a generalized eccentric
...
that yields the short-period corrections as closed-form rational expressions
exact in eccentricity.
...
with $J_3$ through $J_5$ at first order, achieving results that are
...
who introduced the Lie-transform method that superseded Von~Zeipel's
mixed-variable generating function approach.  Deprit's algorithm produces
...
earlier; Campbell and Jefferys~\cite{campbell1970} proved formal equivalence
of the two formulations to all orders.  Hori~\cite{hori1971} subsequently
extended the Lie series approach to non-canonical systems by replacing the
Poisson bracket with Lie derivatives of vector fields, establishing the
theoretical foundation for orbit averaging in arbitrary (non-Hamiltonian)
variable sets---a tradition directly relevant to the present work.
...
non-canonical approaches for short-period correction purity.  All of Lara's
work uses Lie--Deprit transforms with Delaunay variables as the canonical
foundation (results expressed in polar or semi-equinoctial variables),
averaging over mean anomaly, and closed-form eccentricity treatment.  No
complex variable techniques appear in his published corpus.
...
expressed as Fourier series in mean longitude with coefficients involving
Hansen coefficients~\cite{giacaglia1976}; the series are truncated by
coefficient magnitude relative to a tolerance, and certain second-order
contributions (notably the $J_2^2$ effect) have historically been
approximated as power series in eccentricity.  These truncations become the
theory's principal limitation at high eccentricity, where the convergence
of Hansen coefficient expansions slows and the omitted higher-order terms
grow.  At the simplified operational end of the spectrum, the SGP4/SDP4
...
approximately~1~km accuracy at epoch with 1--3~km/day growth, sufficient for
...
Cefola~\cite{sanjuan2022} produced the first second-order closed-form $J_2$
...
geopotential case ($0 \leq |m| \leq n$) these are finite polynomials in
eccentricity---the Fourier series terminates, requiring no truncation.
...
(GEqOE)~\cite{bau2021}.  GEqOE consists of six elements: the generalized
mean motion~$\nu$, generalized eccentricity projections $p_1$ and $p_2$,
generalized mean longitude~$L$, and inclination parameters $q_1,\,q_2$
identical to classical equinoctial.  The generalized eccentric
longitude~$K$ satisfies a generalized Kepler equation
$L = K + p_1\cos K - p_2\sin K$,
with Jacobian $dL/dK = 1 - p_1\sin K - p_2\cos K = r/a$---extending the
$r/a$ Jacobian property from classical to generalized elements.  When the
disturbing potential vanishes, all quantities reduce exactly to classical
equinoctial elements.  The non-osculating ellipse defined by GEqOE produces significantly
improved propagation accuracy compared to classical equinoctial elements
in special-perturbation (numerical integration) mode~\cite{bau2021}.
...
element set with an intermediate anomaly.

%%% Complex variable methods — the gap %%%
...
introduced~\cite{giacaglia1976}.  Extensive
computational literature exists for evaluating these coefficients via
recurrence relations or hypergeometric series, but the contour integral is
never exploited for short-period kernel construction.
...
of the eccentricity parameter---fundamentally different from working in the
complex $F = e^{if}$ plane.  It is well known~\cite{jefferys1971} that the equation of the center
$f - \ell$ cannot be integrated in closed form within the algebra of
trigonometric functions.  The
residue approach in $F$ adopted here circumvents this algebraic obstruction
by operating in the algebra of Laurent series rather than trigonometric
functions.  The tangent-half-angle substitution $t = \tan(f/2)$, widely used
in closed-form orbit averaging (e.g., by
de~Saedeleer~\cite{desaedeleer2005} and Lara~\cite{lara2020}), is
mathematically related to $F = e^{if}$ through $t = -i(F-1)/(F+1)$, but no
prior work in the celestial mechanics literature connects the
tangent-half-angle approach to contour integration or residue methods.
...
closed-form rational functions of the eccentricity parameter~$q$---exact in
eccentricity, with no series truncation, no integral-table lookup, and no
recurrence relation.
...
regimes, despite lacking second-order secular corrections.  This accuracy
advantage is directly relevant to conjunction screening pipelines, where a
propagator combining sub-kilometer accuracy with analytical evaluation cost
reduces the false-positive rate of geometric conjunction filters while
