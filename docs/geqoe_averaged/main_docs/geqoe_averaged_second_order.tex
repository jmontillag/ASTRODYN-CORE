...
$(F+q)^2(1+qF)^2$.
\item[(S3)] The first-order short-period correction $\bm u_1$ is a
\emph{quasi-rational} function of $F$: for zero-mean harmonics (no secular
rate), $\bm u_1$ is purely rational (polynomial + poles at $F = -q$,
$F = -1/q$, and $F = 0$).  For the two secular-rate harmonics per degree
($\Psi$\,\texttt{m=0} and $\Omega$\,\texttt{m=0} at $n=2$; varies for
$n \ge 3$), the complete first-order antiderivative also contains a
$\log(1+qF)$ term.  In the implementation, the rational-only part is
retained, which defines a self-consistent near-identity transformation
accurate to $O(\eps^2)$ in the round-trip (see the master note for
details).
\end{enumerate}
...
However, the second-order \emph{secular rates} $\bar{\bm f}_2$
are sensitive to the first-order log terms through the cross-coupling
$\langle (\partial\bm f_1/\partial\bm z)\,\bm u_{1,\text{log}} \rangle_M$.
If the log terms are dropped from $\bm u_1$, this coupling is lost,
introducing an $O(\eps^2)$ error in the long-period rates.  At second
order it is therefore advisable to include the $\log(1+qF)$ correction
in $\bm u_1$ for the secular-rate harmonics, or equivalently to
add a compensating constant to $\bar{\bm f}_2$.
\end{enumerate}
