...
$\bm u_2$ reducing the osculating reconstruction error from $O(\eps)$ to
$O(\eps^2)$, and (iii) a second-order inverse map eliminating the
...
error source is the $O(\eps)$ magnitude of the short-period correction itself
(zonal scaling slope $= 0.998$). A second-order theory would push the
reconstruction error to $O(\eps^2)$, which for the dominant $J_2$ perturbation
...
$(F+q)^2(1+qF)^2$.
\item[(S3)] The first-order short-period correction $\bm u_1$ is a rational
function of $F$: polynomial + simple poles at $F = -q$, $F = -1/q$,
and higher poles at $F = 0$.
\end{enumerate}
...
\item $\bm u_1$, $v_1$: rational functions of $F$ with poles at $0$, $-q$,
$-1/q$ (from the first-order solution).
...
\item The log-term cancellation argument still holds: the integrand has
zero $M$-average by construction, so log terms cannot appear.
\end{enumerate}
