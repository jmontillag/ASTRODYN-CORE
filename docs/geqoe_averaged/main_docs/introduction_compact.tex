...
catalogs~\cite{rivero2025,rivero2025b}, long-term orbit evolution for debris
...
Kozai~\cite{kozai1959} obtained equivalent closed-form
results via an independent method, using Keplerian elements and the
...
and closed-form integration via dedicated trigonometric
propositions---the closest prior art in \emph{scope}
(generic zonal, closed-form) but fundamentally different in
\emph{technique}.  Mahajan and Alfriend~\cite{mahajan2019} developed a
complementary approach treating arbitrary dominant harmonics for
irregular bodies.
...
classical equinoctial elements.  Related $J_2$ equinoctial elements preserve uncertainty realism far
longer than Cartesian or standard equinoctial
representations~\cite{aristoff2021}; Hernando-Ayuso et
al.~\cite{hernandoayuso2023} showed that GEqOE outperforms all
predecessors in this regard.  A generalized eccentric
...
As has long been observed in the perturbation theory
literature~\cite{jefferys1971}, the equation of the center $f - \ell$
cannot be integrated in closed form within the algebra of trigonometric
functions, so that real-variable averaging methods must typically rely on
integral tables, auxiliary substitutions, or truncated series.  The residue approach in $F = e^{if}$
...
closed-form orbit averaging (e.g., by Lara~\cite{lara2020}), is
...
propagator significantly influences the false-positive rate of geometric
conjunction filters~\cite{rivero2025,rivero2025b}, so a propagator combining
...
propagation, where GEqOE's established advantage in maintaining uncertainty
