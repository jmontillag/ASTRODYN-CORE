...
the thrust-law runtime parameters. On top of these exact arc-end
sensitivities, the current code base now includes an initial multiple-shooting
layer that assembles continuity constraints, terminal residuals, and a
minimum-propellant objective without introducing a separate dynamics backend,
as well as a first constrained-optimization adapter based on SciPy's
\texttt{trust-constr}. The optimization layer is already structured around
explicit terminal-constraint and smoothness-penalty specifications so that the
same transcription can later be reused with richer sparse NLP backends or
alternative weighted objectives.
...
\begin{itemize}
    ...
    variables, and
    \item a working minimum-propellant prototype solve path.
\end{itemize}
...
\emph{outside} the current implementation is equally important: sampled
path-constraint handling along the arcs, richer weighted objectives (for
example time-optimal or power-aware formulations), and alternative sparse NLP
backends beyond the present SciPy prototype. These remaining items are
architectural extensions, not blockers for the current maneuvering workflow.
...
orbital-energy response. Third, a synthetic position-fit experiment solves for
an initial state and piecewise-constant low-thrust coefficients that best
explain noisy inertial position samples, providing a first maneuver-aware
estimation prototype built directly on the new propagation machinery.
