"""Compatibility shim for the derivative-chain math utilities.

Tries to import the C++-accelerated versions from ``math_cpp``.
Falls back to pure-Python implementations that are bit-identical to the
legacy ``extra_utils/math_utils.py``.
"""
...
try:
    ...
    # ------------------------------------------------------------------ #
    # Pure-Python fallback (exact copy of the legacy formulas)            #
    # ------------------------------------------------------------------ #

    def derivatives_of_inverse(  # type: ignore[misc]
    ...
    def derivatives_of_inverse_wrt_param(  # type: ignore[misc]
    ...
    def derivatives_of_product(  # type: ignore[misc]
        a_vector: np.ndarray, do_one: bool = False
    ):
        m = len(a_vector)
        n = m - 1
    ...
    def derivatives_of_product_wrt_param(  # type: ignore[misc]
        a_vector, a_d_vector, do_one=False
    ):
        m = len(a_vector)
        n = m - 1
